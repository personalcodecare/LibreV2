...
contract MasterChef is Ownable,ReentrancyGuard {
    ...
    function getPoolInfo(uint256 _pid)external view returns(address lp, uint256 allocPoint, uint256 lastRewardBlock, address[]memory lpPath, uint256 accLibPerShare, bool isETHpair){
        PoolInfo storage pool = poolInfo[_pid];
        return (address(pool.lpToken), pool.allocPoint, pool.lastRewardBlock, pool.lpPath, pool.accLibPerShare, pool.isETHPair);
    }
    ...
    function setNewFarm(address payable _newFarm) public onlyOwner {
        newFarm = MasterChef(_newFarm);
    }
    function setOldFarm(address payable _oldFarm) public onlyOwner {
        oldFarm = MasterChef(_oldFarm);
    }
    function migrate(uint256 _pid) public {
        require(address(newFarm) != address(0), "migrate: no new farm");
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        if(_pid>0){
            pool.lpToken.safeTransfer(address(newFarm), user.amount);
        } 
        else{
            safeLibreTransfer(address(newFarm),user.amount);
            totalStaked = totalStaked.sub(user.amount);
        }
        migrateReceiver(msg.sender,address(pool.lpToken), user.amount, user.rewardDebt);
        user.amount = 0;
        user.rewardDebt = 0;
    }
    function migrateReceiver(address user, address lp, uint256 _amount, uint256 _reward) public {
        require(msg.sender == address(oldFarm), "migrate: must send from old farm");
        uint256 _pid = pidMapping[lp];
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        if(_pid ==0) totalStaked = totalStaked.add(_amount);
        if (user.amount > 0) {
            uint256 pending =
    ...
    function deposit(uint256 _pid, uint256 _amount, uint8 _slippage, bool _revert) public validatePoolByPid(_pid) nonReentrant payable{
        ...
        address[] memory lpPath;
        lpPath[0] = _revert ?pool.lpPath[1]:pool.lpPath[0];
        lpPath[1] = _revert ?pool.lpPath[0]:pool.lpPath[1];
        uint256 token1Amount=_swap(msg.sender, lpPath, pool.isETHPair, _amount,_slippage);
    ...
    function _swap(address payable _to, address[]memory lpPath, bool _isETH, uint256 _amount, uint8 _slippage)private returns (uint256){
        IERC20 token0 = IERC20(lpPath[0]);
        IERC20 token1 = IERC20(lpPath[1]);
        uint256 token0init = _isETH?address(this).balance :token0.balanceOf(address(this));
        if(!_isETH)token0.transferFrom(_to, address(this), _amount);
        ...
        if(_isETH) uniRouter.swapExactETHForTokens{value: _amount.div(2)}(minAmount, lpPath, address(this), block.timestamp);
        else uniRouter.swapExactTokensForTokens(_amount.div(2), minAmount, lpPath, address(this), block.timestamp);
       
        uint256 token0Amount = _isETH ?_amount.div(2) :token0init.add(_amount).sub(token0.balanceOf(address(this))); 
    ...
    function _addLibLiquidity(address payable _to, bool _isETH, uint256 _amount, IERC20 token0, IERC20 token1, uint256 token0init, uint256 token1Before,uint256 token0Amount, uint256 token1Amount)private{
        if(_isETH) libreRouter.addLiquidityETH{value: token0Amount}(address(token1),token1Amount,0,0,address(this), block.timestamp);
        else libreRouter.addLiquidity(address(token0), address(token1), token0Amount, token1Amount, 0 , 0, address(this), block.timestamp);
        uint256 token0leftOver = _isETH?address(this).balance.sub(token0init.sub(_amount)):token0.balanceOf(address(this)).sub(token0init); 
    ...
    function _slippageCalculate(IERC20 token0, IERC20 token1, uint256 input, uint8 _slippage)private view returns(uint256){
